...
xcrun swiftc -O -parse-as-library -target "$(uname -m)-apple-macosx14.0" \
    ...
    -o "$app_dir/Contents/MacOS/HistoryScrollPreview"
cat > "$app_dir/Contents/Info.plist" <<'PLIST'
<?xml version="1.0" encoding="UTF-8"?>
<plist version="1.0"><dict>
<key>CFBundleIdentifier</key><string>com.github.nserfan.CodexLimits.scroll-preview</string>
<key>CFBundleName</key><string>History Scroll Preview</string>
<key>CFBundleExecutable</key><string>HistoryScrollPreview</string>
...
pkill -TERM -x HistoryScrollPreview 2>/dev/null || true
for _ in {1..25}; do
    pgrep -x HistoryScrollPreview >/dev/null || break
    sleep 0.2
done
open "$app_dir"
