...
public enum UsageChartStyle {
    ...
    public static func accent(for remaining: Double?, scheme: ColorScheme, selection: UsageAccent = .automatic) -> Color {
        AccentContrast.readable(surfaceTint(for: remaining, scheme: scheme, selection: selection), scheme: scheme)
    }

    public static func surfaceTint(for remaining: Double?, scheme: ColorScheme, selection: UsageAccent = .automatic) -> Color {
        if selection != .automatic { return selection.color(scheme: scheme) }
        ...
        return UsageAccent.mint.color(scheme: scheme)
    }
}
